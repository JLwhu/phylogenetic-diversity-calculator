...
public class SpeciesBetaDiversityDistanceExpectedFracA extends CommunityDiversityDistanceStrategy{

	public SpeciesBetaDiversityDistanceExpectedFracA(Tree phylotree) {
		super(phylotree);
	...
	public double distance(Set<String> A, Set<String> B) {
		...
		double sum =0;
		int startidx = largeSetSize+smallSetSize-total;
		if (startidx<=0) startidx = 1;
		for (int i=startidx;i<=smallSetSize;i++){
			double combin = Combination.getCnm(largeSetSize, i) * Combination.getCnm(total-largeSetSize, smallSetSize-i);
			sum = sum +i
					* combin;
		}
		
		double denominator = Combination.getCnm(total, smallSetSize);
