...
public class ClusterEvaluation implements Serializable, RevisionHandler {
	...
	/**
	 * Return an array of cluster assignments corresponding to the most recent
	...
	/**
	 * Return the log likelihood corresponding to the most recent set of
	 * instances clustered.
	 * 
	 ...
	 *            the set of instances to cluster
	 * @param testFileName
	 *            the name of the test file for incremental testing, if "" or
	 *            null then not used
	 * 
	 * @throws Exception
	 *             if something goes wrong
	 ...
	 *            null then not used
	 * @param outputModel
	 *            true if the clustering model is to be output as well as the
	 *            stats
	 * 
	 * @throws Exception
	 *             if something goes wrong
	 ...
	 * Assumes that m_Clusterer has been trained and tested on inst (minus the
	 * class).
	 * 
	 * @param inst
	...
	private void evaluateClustersWithRespectToClass(Instances inst,
		...
		String matrixString = toMatrixString(counts, clusterTotals,
				new Instances(inst, 0));
		...
		for (i = 0; i < m_numClusters; i++) {
			if (clusterTotals[i] > 0) {
				...
				if (best[i] < 0) {
					...
					m_clusteringResults.append(
							inst.classAttribute().value((int) best[i])).append(
							"\n");
				}
			}
		}
		m_clusteringResults.append("\nIncorrectly clustered instances :\t"
				...
				+ "\t"
				+ (Utils.doubleToString(
						(best[m_numClusters] / numInstances * 100.0), 8, 4))
				+ " %\n");
	 ...
	 * 
	 * @param counts
	 *            the counts of classes for each cluster
	 * @param clusterTotals
	 *            total number of examples in each cluster
	 * @param inst
		...
		for (int i = 0; i < counts[0].length; i++) {
			...
			for (int j = 0; j < m_numClusters; j++) {
				if (clusterTotals[j] > 0) {
					ms.append(" ").append(
							Utils.doubleToString((double) counts[j][i], Cwidth,
									0));
				}
			}
			ms.append(" | ").append(inst.classAttribute().value(i))
					.append("\n");
		}
	...
	public static String evaluateClusterer(Clusterer clusterer, String[] options)
		...
		try {
			...
			if (trainFileName.length() == 0) {
				...
				if (testFileName.length() == 0) {
					throw new Exception(
							"No training file and no test file given.");
				}
			} else {
				if ((objectInputFileName.length() != 0)
				...
				if (classString.length() != 0) {
					...
					if (theClass != -1) {
						if (doXval || testFileName.length() != 0)
							throw new Exception(
									"Can only do class based evaluation on the "
											+ "training data");

						if (objectInputFileName.length() != 0)
							throw new Exception(
									"Can't load a clusterer and do class based "
											+ "evaluation");
		...
		if (objectInputFileName.length() != 0) {
			...
			// clusterer = (Clusterer)
			// SerializationHelper.read(objectInputFileName);
			...
			if (theClass == -1) {
				...
				if (updateable) {
					Instances clusterTrain = Filter.useFilter(train,
							removeClass);
					...
					while (source.hasMoreElements(train)) {
						...
						((UpdateableClusterer) clusterer)
								.updateClusterer(clusterTrainInst);
					}
					((UpdateableClusterer) clusterer).updateFinished();
				} else {
					Instances clusterTrain = Filter.useFilter(
							source.getDataSet(), removeClass);
					clusterer.buildClusterer(clusterTrain);
					trainHeader = clusterTrain;
				}
		...
		if (testFileName.length() != 0) {
			...
			if (!trainHeader.equalHeaders(testStructure)) {
				throw new Exception(
						"Training and testing data are not compatible");
			}
	 ...
	 * @return a string describing the results
	 */
	public static String evaluateClustererNew(Clusterer clusterer, String[] options)
			throws Exception {
	...
	/**
	 * Performs a cross-validation for a DensityBasedClusterer clusterer on a
	 * set of instances.
	 * 
	 * @param clustererString
	...
	public static String crossValidateModel(String clustererString,
		...
		// create clusterer
		try {
			clusterer = (Clusterer) Class.forName(clustererString)
					.newInstance();
		} catch (Exception e) {
	 ...
	 * @throws Exception
	 *             if statistics can't be generated.
	 */
	 ...
	 *             if statistics can't be generated.
	 */
	public static String printClusterStats(Clusterer clusterer,
			Instances structure) throws Exception {
		StringBuffer text = new StringBuffer();
		int i = 0;
		int cnum;
		double loglk = 0.0;
		...
		/*
		 * // count the actual number of used clusters int count = 0; for (i =
		 * 0; i < cc; i++) { if (instanceStats[i] > 0) { count++; } } if (count
		 * > 0) { double[] tempStats = new double [count]; count=0; for
		 * (i=0;i<cc;i++) { if (instanceStats[i] > 0) { tempStats[count++] =
		 * instanceStats[i]; } } instanceStats = tempStats; cc =
		...
		for (i = 0; i < cc; i++) {
			if (instanceStats[i] > 0) {
				text.append(Utils
						.doubleToString((double) i, clustFieldWidth, 0)
						+ "      "
						+ Utils.doubleToString(instanceStats[i],
								numInstFieldWidth, 0)
						+ " ("
	 ...
	 *             if cluster assignments can't be printed
	 */
	public static String printClusterings(Clusterer clusterer,
			Instances structure, Range attributesToOutput)
			throws Exception {
	...
	private static String makeOptionString(Clusterer clusterer,
		...
		optionsText
				.append("\tOutput synopsis for clusterer (use in conjunction "
						+ " with -h)\n");
		...
		optionsText
				.append("\tSet class attribute. If supplied, class is ignored");
	 ...
	 * @throws Exception
	 *             if there is a problem reflecting on the clusterer
	 */
	...
	public boolean equals(Object obj) {
		...
		String clusteringResults1 = m_clusteringResults.toString().replaceAll(
				"Elapsed time.*", "");
		String clusteringResults2 = cmp.m_clusteringResults.toString()
	...
	public static void main(String[] args) {
		try {
			...
			args[0] = "";
			Clusterer newClusterer = AbstractClusterer.forName(ClustererString,
					null);
